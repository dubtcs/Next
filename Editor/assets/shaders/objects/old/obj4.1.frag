...
// float DGetShadow(uint index) //  Directional
// {
...
// }
float PGetShadow(uint index)
{
    vec3 positionDif = pWorldPosition - lights[1].Position;
    float closestDepth = texture(shadowMap, positionDif).r;
    closestDepth *= 25.0; // far plane
    float currentDepth = length(positionDif);
...
vec3 PointLight(uint index)
{
    ...
    return (diffuse + specular) * (1.0 - PGetShadow(index));
}
